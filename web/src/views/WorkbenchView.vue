  ...
  <div class="workbench-page">
    <section class="workbench-hero">
      ...
      <div class="hero-actions">
        <el-button v-if="permissionState === 'default'" :icon="Bell" plain @click="requestNotificationPermission">
          开启系统提醒
        </el-button>
        <el-button type="primary" :icon="Plus" @click="openCreate(todayKey)">新建待办</el-button>
      </div>
    </section>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑待办' : '新建待办'" width="560px" destroy-on-close class="task-dialog">
...
@media (max-width: 850px) { .workbench-hero { align-items: flex-start; flex-direction: column; padding: 22px; }.workbench-grid { grid-template-columns: 1fr; }.task-board { min-height: 480px; }.workbench-side { grid-template-columns: minmax(310px,1fr) minmax(260px,.8fr); } }
@media (max-width: 620px) { .metric-grid { grid-template-columns: 1fr 1fr; }.metric-card { min-height: 94px; padding: 13px; }.workbench-side { grid-template-columns: 1fr; }.workbench-card-head { align-items: stretch; flex-direction: column; }.task-search { width: 100%; }.task-board { padding: 17px; }.task-filter-row { overflow-x: auto; }.task-filter-row button { flex: 0 0 auto; }.task-item { grid-template-columns: minmax(0,1fr) auto; gap: 8px 10px; }.task-details { grid-column: 1 / -1; grid-row: 2; padding: 7px 0 0 32px; border-top: 1px solid #edf2f7; border-left: 0; }.task-actions { grid-column: 2; grid-row: 1; }.dialog-form-grid { grid-template-columns: 1fr; }.hero-actions { width: 100%; flex-wrap: wrap; }.hero-actions :deep(.el-button) { flex: 1; margin-left: 0; } }
</style>
